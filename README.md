...
This downloads the matching-arch release tarball and runs the `Makefile`
it ships (generated by `fyne-cross`) to install the binary, icon and
applications-menu launcher under `/usr/local`, then drops a shortcut on
each user's Desktop. Needs `make` on the machine. Variations:
